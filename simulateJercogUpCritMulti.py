...
p['timeAfterSpiked'] = paradoxicalKickTimes[-1] + paradoxicalKickDurs[-1]
...
estimatedDuration = paradoxicalKickTimes[-1] + paradoxicalKickDurs[-1] + p['timeToSpike']
estimatedNBinCenters = int(np.round(estimatedDuration / p['dtHistPSTH'])) - 1

PSTHExc = np.empty((nTrials, estimatedNBinCenters))
PSTHInh = np.empty((nTrials, estimatedNBinCenters))
...
for trialInd in range(nTrials):
    ...
    if pL['paradoxicalKickInh']:  # plot the current injection region
        ...
        PSTHExc[trialInd, :] = R.FRExc
        PSTHInh[trialInd, :] = R.FRInh
...
frDF = pd.concat((paraFRExcsDF, paraFRInhsDF, compFRExcsDF, compFRInhsDF, ))
frDF['trialIndex'] = frDF.index
frDF.to_csv(os.path.join(os.getcwd(), p['nameSuffix'] + '_results.csv'))

savePath = os.path.join(os.getcwd(), p['nameSuffix'] + '_PSTH.npz')
np.savez(savePath, PSTHExc=PSTHExc, PSTHInh=PSTHInh)
